...
public class Fabrica {
	...
	private Map<Integer, Figurita> figuritasTradicionales;
	private Map<Integer, FiguritaTOP10> figuritasTOP10PorMundial;
	Fabrica() {
		...
		figuritasTradicionales = new HashMap<Integer, Figurita>();
		figuritasTOP10PorMundial = new HashMap<Integer, FiguritaTOP10>();
		//Genero todas las figuritas tradicionales
		int numeroId = 0;
		for (int i = 0; i < paisesParticipantes.length; i++) {
			for (int j = 0; j < lugaresPorPais; j++) {
				numeroId += 1;
				Figurita figurita = new Figurita(paisesParticipantes[i], "JugadorDe"+ paisesParticipantes[i] + j, this.calcularValorBase(paisesParticipantes[i], j), j, numeroId);
				figuritasTradicionales.put(numeroId, figurita);
			}
		}
		//Genero todas las figuritas TOP10
		for (int i = 0; i < listadoDeMundialesTop10.length; i++) {
			for (int j = 0; j < 2; j++) {
				numeroId += 1;
				String[] mundial = listadoDeMundialesTop10[i].split("'");
				String balon = j == 0 ? "Oro" : "Plata";
				int aumento = j == 0 ? 20 : 10;
				int valorFinal = this.calcularValorBase(paisesParticipantes[i], j) + (this.calcularValorBase(paisesParticipantes[i], j) * aumento / 100);
			    FiguritaTOP10 figurita = new FiguritaTOP10(paisesParticipantes[i], "JugadorTOPDe"+ paisesParticipantes[i] + j, valorFinal, j, numeroId, mundial[0], mundial[1], balon);
				figuritasTOP10PorMundial.put(numeroId, figurita);
			}
		}
	}
	...
	Album crearAlbumWeb() {
		AlbumWeb albumWeb = new AlbumWeb("Remera de la seleccion", "Web", this.generarPaisesClasificados());
		return albumWeb;
	}

	Album crearAlbumExtendido() {
		AlbumExtendido albumExtendido = new AlbumExtendido( "Pelota y viaje", "Extendido", this.generarPaisesClasificados(), listadoDeMundialesTop10);
		return albumExtendido;
	}

	Album crearAlbumTradicional() {
		AlbumTradicional albumTradicional = new AlbumTradicional("Pelota", "Tradicional", this.generarPaisesClasificados());
		return albumTradicional;
	}
	
	Map<Integer, Figurita> darFiguritasTradicionalesTotales(){
		return this.figuritasTradicionales;
	}
	
	Map<Integer, FiguritaTOP10> darFiguritasTOP10Totales(){
		return this.figuritasTOP10PorMundial;
	}
	
	List<Figurita> generarSobre(int cantFigus) {
		List<Figurita> sobre = new ArrayList<Figurita>();
		Collection<Figurita> figuritas = figuritasTradicionales.values();
		Object[] figusArray = figuritas.toArray();
		while(cantFigus > 0) {
			int figuritaNumero = this.random.nextInt(figuritasTradicionales.size());
			Figurita figurita = (Figurita) figusArray[figuritaNumero];
			sobre.add(figurita);
			cantFigus--;
		}
		return sobre;
	...
	List<Figurita> generarSobreTop10(int cantFigus) {
		List<Figurita> sobre = new ArrayList<Figurita>();
		Collection<FiguritaTOP10> figuritasTOP10 = figuritasTOP10PorMundial.values();
		Object[] figusArray = figuritasTOP10.toArray();
		while(cantFigus > 0) {
			int figuritaNumero = this.random.nextInt(figuritasTOP10PorMundial.size());
			Figurita figurita = (Figurita) figusArray[figuritaNumero];
			sobre.add(figurita);
			cantFigus--;
		}
	...
	public String[] darPremios() {
		return this.premiosInstantaneos;
	}
	...
	private String[] generarPremiosParaSorteoInstantaneo() {//Una vez completado album tradicional se hace el sorteo
		return new String[]{
